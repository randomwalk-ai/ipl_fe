<script lang="ts">
	import { BellRingIcon, SettingsIcon } from '@lucide/svelte';
	...
	import { addHoursToDate, timeAgo } from '$lib/utils';
	import { ArrowLeftIcon } from '@lucide/svelte';
	import BannersAndSlogans from './BannersAndSlogansOld.svelte';
	import PoliceMonitoring from './PoliceMonitoringOld.svelte';
	...
	let bannerAndSlogansConfig = $state({
	...
	// Add this state variable near the top of your script section
	let selectedCardType = $state('');

	// Add this state variable to track if we're in card detail view
	let showingCardDetailView = $state(false);
	...
	function showFilteredView(query: string) {
		selectedCardType = query;
		selectedQuery = query;
		showingFilteredView = true;
		showingCardDetailView = true;
	}
	...
	function showBannerQueriesView(cardType: string) {
	...
	function showPoliceView() {
		selectedCardType = 'police';
		showingPoliceView = true;
		showingCardDetailView = true;
	}

	// Function to get the current view title
	function getCurrentViewTitle() {
		if (showingFilteredView) {
			return `${selectedQuery} Alerts`;
		} else if (showingBannerQueriesView) {
			return selectedCardType;
		} else if (showingBannerAlertsView) {
			return `${selectedCardType} - ${selectedCamera}`;
		} else if (showingPoliceView) {
			return "No Police Alerts";
		} else {
			return "Recent Alerts & Events";
		}
	}
</script>
	...
	<ScrollArea class="flex-1">
		<CardContent class="p-4">
			...
			{:else if showingBannerQueriesView || showingBannerAlertsView}
				<!-- Use the simplified BannersAndSlogans component -->
				<BannersAndSlogans
					...
					onCardSelect={(cardType) => {
						...
						showBannerQueriesView(cardType);
					}}
				/>
			{:else if showingPoliceView}
				...
				<PoliceMonitoring
					...
					selectedCardType={selectedCardType}
					isSelected={selectedCardType === 'police'}
				/>
			{:else}
				...
				{:else}
					<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
						...
						{#each additionalAlertCards as card}
							<div
								class="flex flex-col cursor-pointer items-center justify-center rounded-md border p-4 shadow-sm transition-all hover:shadow-md dark:border-gray-700 {selectedCardType === card.alertTitle ? 'ring-2 ring-primary' : ''}"
								role="button"
								tabindex="0"
