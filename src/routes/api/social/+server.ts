	...
	const result = await db.transaction(async (tx) => {
		...
		const totalStatsPromise = tx
			.select({
				...
				positive: count(sql`CASE WHEN ${tweets.sentiment} = 'positive' THEN 1 END`).as(
					'positive_count'
				),
				negative: count(sql`CASE WHEN ${tweets.sentiment} = 'negative' THEN 1 END`).as(
					'negative_count'
				),
				neutral: count(sql`CASE WHEN ${tweets.sentiment} = 'neutral' THEN 1 END`).as(
					'neutral_count'
				)
			})
		...
		const playerOverallStats = playerOverallStatsResult[0] ?? {
			total: 0,
			positive: 0,
			neutral: 0,
			negative: 0
