					...
					{#each blockData as block (block.name)}
						...
						>
							<img
								src="/camera-lens.png"
								alt={block.name}
								class={`block h-2 w-2 origin-center transform transition-transform duration-200 ease-in-out hover:scale-110 dark:invert ${activeBlock === block ? 'rounded-full ring-2 ring-blue-500' : ''}`}
							/>
						</div>
					{/each}
				</div>
