...
export const detections = pgTable(
	...
	(table) => [
		index('ix_detections_id').using('btree', table.id.asc().nullsLast().op('int4_ops')),
		foreignKey({
...
export const analytics = pgTable(
	...
	(table) => [
		index('ix_analytics_id').using('btree', table.id.asc().nullsLast().op('int4_ops')),
		foreignKey({
			columns: [table.cameraId],
			foreignColumns: [cameras.id],
			name: 'analytics_camera_id_fkey'
		})
	]
);
