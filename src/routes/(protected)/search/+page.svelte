<script lang="ts">
	...
	async function performSearch(options?: {
		...
		if (isSimilaritySearch && currentImageFile) {
			console.log(
				...
				'and filters:', { cameras: cameras.map(c=>c.name), labels: selectedLabels, after: afterTimestamp, before: beforeTimestamp, limit, conf, maxDistance, sortBy }
			);
			formData.append('image', currentImageFile, currentImageFile.name);
		} else if (query && query.trim() !== '') {
            console.log(
				'Performing STANDARD TEXT search with query:', query,
                // *** Update log to include maxDistance ***
                'and filters:', { cameras: cameras.map(c=>c.name), labels: selectedLabels, after: afterTimestamp, before: beforeTimestamp, limit, conf, maxDistance, sortBy }
            );
    ...
    function getSortLabel(): string {
        switch(sortBy) {
            ...
            case 'score_desc': return 'Most Similar/Confident First';
            case 'score_asc': return 'Least Similar/Confident First';
...
<ScrollArea class="h-full overflow-x-visible p-4 text-gray-200">
	...
	<div class="mb-8">
		...
		>
			...
			<div class="w-full flex-grow">
				...
				<Input
					...
					bind:value={query}
                    oninput={() => { if (selectedImageFile) clearSelectedImage(); }}
					disabled={isLoading}
				/>
			</div>
        ...
        <p class="mb-4 text-xs text-gray-400 italic">
            Filters (Date, Labels, Cameras, Confidence, Max Distance, Sort) apply to both text and image searches.
        </p>
        <div class="grid grid-cols-1 items-end gap-x-4 gap-y-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
			<div class="pt-1">
			...
			<div>
				...
				>
					...
					<Select.Content class="max-h-[40vh] border-gray-700 bg-gray-800 text-gray-200">
			...
			<div>
                ...
                <div class="relative">
                    <Input
                        ...
                        defaultValue="85"
                        class={cn(
                            'w-full border-gray-700 bg-gray-800 text-sm text-white focus:ring-blue-500 pr-8', // Add padding for icon
                            standardFieldsDisabled && 'cursor-not-allowed opacity-50'
                        )}
                        bind:value={conf}
                    ...
                    />
                     <Tooltip.Root>
                        ...
                        <Tooltip.Content class="max-w-xs border-gray-700 bg-gray-900 text-gray-300">
                            Minimum similarity confidence score (0-100). Only results with confidence >= this value will be returned. Provided by the semantic search service.
                        </Tooltip.Content>
                    </Tooltip.Root>
                </div>
			</div>
            ...
            <div>
                ...
                >
                    ...
                    <Select.Content class="border-gray-700 bg-gray-800 text-gray-200">
                        <Select.Item value="date_desc" class="data-[highlighted]:bg-blue-600">Newest First</Select.Item>
                        <Select.Item value="date_asc" class="data-[highlighted]:bg-blue-600">Oldest First</Select.Item>
                        <Select.Item value="score_desc" class="data-[highlighted]:bg-blue-600">Most Similar/Confident First</Select.Item>
                        <Select.Item value="score_asc" class="data-[highlighted]:bg-blue-600">Least Similar/Confident First</Select.Item>
                    </Select.Content>
                </Select.Root>
            </div>
		</div>
    ...
    <div class="mb-6 -mt-4 text-xs text-gray-500 flex items-center gap-1">
        <InfoIcon class="size-3 shrink-0" />
        <span>
            Similarity is often calculated from distance (e.g., 1 - distance). Low distance ≈ High similarity.
            Confidence and Distance filters can be used together.
        </span>
    </div>
                                ...
                                <button
                                    ...
                                    onclick={(e) => {
                                        ...
                                        if (!isLoading && result.thumbnail) {
                                            ...
                                            searchError = 'Event thumbnail is missing, cannot perform similarity search.';
                                        }
                                    }}
                                    ...
                                    {#if typeof result.search_distance === 'number' || typeof result.search_confidence === 'number'}
                                         ...
                                         >
                                            {#if typeof result.search_distance === 'number'}
                                                ...
                                                <span class="text-blue-300" title={`Lower distance is more similar. Raw: ${result.search_distance}`}>
                                                    Dist: {distanceFormatted}
                                                </span>
                                                {#if typeof result.search_confidence === 'number'}<span class="text-gray-400"> | </span>{/if}
                                            {/if}
                        ...
                        <Dialog.Header>
                         ...
                         <Tabs.Root value="details" class="w-full pt-2">
                            <Tabs.List class="border-b border-gray-700">
                                <Tabs.Trigger value="details" class="data-[state=active]:border-blue-500 data-[state=active]:text-blue-400">Details</Tabs.Trigger>
                                {#if snapshotUrl && result.has_snapshot}
                                    <Tabs.Trigger value="snapshot" class="data-[state=active]:border-blue-500 data-[state=active]:text-blue-400">Snapshot</Tabs.Trigger>
                                {:else if result.has_snapshot}
                                    <Tabs.Trigger value="snapshot" disabled class="cursor-not-allowed text-gray-600">Snapshot (Unavailable)</Tabs.Trigger>
                                {/if}
                                {#if clipUrl && result.has_clip}
                                    <Tabs.Trigger value="video" class="data-[state=active]:border-blue-500 data-[state=active]:text-blue-400">Video</Tabs.Trigger>
                                {:else if result.has_clip}
                                    <Tabs.Trigger value="video" disabled class="cursor-not-allowed text-gray-600">Video (Unavailable)</Tabs.Trigger>
                                {/if}
                            </Tabs.List>
                            <Tabs.Content value="details" class="mt-4 text-sm outline-none focus:ring-0">
                                <dl class="grid grid-cols-1 gap-x-6 gap-y-3 md:grid-cols-2">
                                    <!-- Standard Details -->
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Event ID</dt><dd class="mt-1">{result.id}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Camera</dt><dd class="mt-1">{result.camera}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Label</dt><dd class="mt-1">{result.label} {#if result.sub_label}({result.sub_label}){/if}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Detection Confidence</dt><dd class="mt-1">{result.top_score ? `${Math.round(result.top_score * 100)}%` : 'N/A'}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Start Time</dt><dd class="mt-1">{formatTimestamp(result.start_time)}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">End Time</dt><dd class="mt-1">{result.end_time ? formatTimestamp(result.end_time) : 'N/A'}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Has Clip</dt><dd class="mt-1">{result.has_clip ? 'Yes' : 'No'}</dd></div>
                                    <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Has Snapshot</dt><dd class="mt-1">{result.has_snapshot ? 'Yes' : 'No'}</dd></div>
                                    {#if result.source_instance}<div class="sm:col-span-1"><dt class="font-medium text-gray-400">Source Instance</dt><dd class="mt-1">{frigateInstances.find(inst => inst.url === result.source_instance)?.name ?? result.source_instance}</dd></div>{/if}

                                    <!-- Semantic Search Specific Details -->
                                    {#if typeof result.search_distance === 'number'}
                                        <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Search Distance</dt><dd class="mt-1 text-blue-300">{result.search_distance.toFixed(4)} (Lower is more similar)</dd></div>
                                    {/if}
                                    {#if typeof result.search_confidence === 'number'}
                                        <div class="sm:col-span-1"><dt class="font-medium text-gray-400">Search Confidence</dt><dd class="mt-1 text-green-300">{result.search_confidence.toFixed(1)}%</dd></div>
                                    {/if}

                                    <!-- Additional Data if available -->
                                    {#if result.data?.type}<div class="sm:col-span-1"><dt class="font-medium text-gray-400">Type</dt><dd class="mt-1">{result.data.type}</dd></div>{/if}
                                    {#if result.data?.score && result.data.score !== -1}<div class="sm:col-span-1"><dt class="font-medium text-gray-400">Score (Raw)</dt><dd class="mt-1">{result.data.score.toFixed(3)}</dd></div>{/if}
                                    {#if result.data?.attributes && result.data.attributes.length > 0}<div class="sm:col-span-2"><dt class="font-medium text-gray-400">Attributes</dt><dd class="mt-1"><pre class="whitespace-pre-wrap text-xs">{JSON.stringify(result.data.attributes, null, 2)}</pre></dd></div>{/if}
                            ...
                            <Tabs.Content value="snapshot" class="mt-4 outline-none focus:ring-0">
                                <div class="relative">
                                    <img src={snapshotUrl} alt="Snapshot for event {result.id}" class="mx-auto max-h-[70vh] max-w-full rounded object-contain" loading="lazy" onerror={(e) => { (e.target as HTMLImageElement).nextElementSibling?.setAttribute('style', 'display: block;'); }} />
                                    <p class="absolute inset-x-0 top-0 mt-2 text-center text-red-400" style="display: none;">Failed to load snapshot...</p>
                                </div>
                            </Tabs.Content>
                            {/if}
                             {#if clipUrl && result.has_clip}
                            <Tabs.Content value="video" class="mt-4 outline-none focus:ring-0">
                                <div class="relative">
                                    <video src={clipUrl} controls preload="metadata" class="max-h-[70vh] w-full rounded bg-black" onerror={(e) => { (e.target as HTMLVideoElement).nextElementSibling?.setAttribute('style', 'display: block;'); }}>Your browser does not support the video tag.</video>
                                    <p class="absolute inset-x-0 top-0 mt-2 text-center text-red-400" style="display: none;">Failed to load video clip...</p>
                                </div>
                            </Tabs.Content>
                            {/if}
                         </Tabs.Root>
